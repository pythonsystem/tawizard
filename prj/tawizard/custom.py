...
def import_dir(path, verbose=True):
    ...
    for d in dirs:
        ...
        # only look in directories which are valid tawizard categories
        if dirname not in [*tawizard.Category]:
            if verbose:
                print(f"[i] Skipping the sub-directory '{dirname}' since it's not a valid tawizard category.")
            continue
        ...
        for module in glob(abspath(join(path, dirname, "*.py"))):
            ...
            # figure out which of the modules functions to bind to tawizard
            ...
            if module_name not in tawizard.Category[dirname]:
                tawizard.Category[dirname].append(module_name)
...
import_dir.__doc__ = \
"""
Import a directory of custom indicators into tawizard
...
together with the existing tawizard functions just like if they were part of
tawizard.

If you at some late point would like to push them into the tawizard library
...
available in categories under the tawizard-folder. The only difference will
...
like all other native indicators in tawizard, including help functions.
"""
